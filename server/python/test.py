...
def final(chords, path):
	...
	for index in num_notes:
		event1 = midi.NoteOnEvent(tick=0, channel=0, data=[chords[chord_counter][0], 90])
		event2 = midi.NoteOnEvent(tick=0, channel=0, data=[chords[chord_counter][1], 90])
		event3 = midi.NoteOnEvent(tick=0, channel=0, data=[chords[chord_counter][2], 90])
...
chords,path = read_file("../eecs352/harmoni/final_java_code/backend/java_out.txt")
